    ...
    <section id="servicos" className="py-20 px-6 max-w-[1280px] mx-auto">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
        {services.map((service) => (
          ...
          >
            <div className="w-14 h-14 rounded-full bg-primary flex items-center justify-center mb-6 group-hover:scale-110 transition-transform">
        ...
        {/* Call to Action Card */}
        <div className="group p-8 rounded-[2rem] bg-primary/10 border border-primary/20 hover:border-primary hover:shadow-lg transition-all duration-300 relative overflow-hidden flex flex-col">
          <div className="absolute -right-10 -top-10 text-primary/10">
            <Icon name="emergency_home" className="text-[150px]" />
          </div>
          <div className="relative z-10 h-full flex flex-col justify-between">
            <div>
              <h3 className="text-2xl font-bold mb-3 text-text-main">Precisa de outra coisa?</h3>
              <p className="text-text-muted mb-6">Realizamos diversos tipos de serviços personalizados. Entre em contato e descreva sua necessidade.</p>
            </div>
            <a href="#contato" className="w-full py-3 rounded-full bg-primary text-text-main font-bold hover:bg-[#e6e205] transition-colors text-center block">
              Pedir Orçamento
            </a>
          </div>
        </div>
